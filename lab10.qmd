...
# adjust column names
state_pops$abs = state_pops$state
...
```{r}
# format the date
cv_states$date = date(cv_states$date)
#str(cv_states)

# format the state and state abbreviation (abb) variables
cv_states$state = factor(cv_states$state)
cv_states$abs = factor(cv_states$abs)
...
# }
 ...
 cv_states = cv_states %>% mutate(new_cases = cases - lag(cases) , new_deaths = deaths - lag(deaths))
...
### COMPLETE CODE HERE
p1 = ggplot(data = cv_states, aes(x = new_cases)) + geom_histogram() + labs(x = "new cases")
ggplotly(p1)

p2 = ggplot(data = cv_states, aes(x = new_deaths)) + geom_histogram() + labs(x = "new deaths")
ggplotly(p2)
...
cv_states_today_filter |> 
  plot_ly(x = ~pop_density, y = ~deathsper100k,
          type = "scatter", mode = "markers", color = ~state,
          size = ~population, sizes = c(3,100), marker = list(sizemode = 'diameter'),
          hoverinfo = 'text',
...
The peak for deaths is around September 20th 2021 and February 16th 2022. The corresponding peaks for cases are August 16th 2021 and January 10th 2022. That corresponds to roughly a 1 month difference between the peak of new cases, and the peak of new deaths.
...
cv_per100 = cv_states |> filter(date==pick.date) |> select(state, abs, newper100k, cases, deaths) # select data
cv_per100$state_name = cv_per100$state
cv_per100$state = cv_per100$abs
cv_per100$abs = NULL
...
# Extract the data for each state by its abbreviation
cv_per100 = cv_states_today |>  select(state, abs, newper100k, cases, deaths) # select data
cv_per100$state_name = cv_per100$state
cv_per100$state = cv_per100$abs
cv_per100$abs = NULL
